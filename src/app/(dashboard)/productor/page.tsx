...
export default function ProducerDashboardPage() {
  const activeProductions = mockProductions.length;
  const newOffers = mockProductions.flatMap(p => p.offers).filter(o => o.status === 'pendiente' || o.status === 'negociación').length;

  const acceptedOffers = mockProductions.flatMap(p => p.offers).filter(o => o.status === 'aceptada');
  const projectedIncome = acceptedOffers.reduce((sum, offer: Offer) => sum + (offer.amount * offer.pricePerUnit), 0);
